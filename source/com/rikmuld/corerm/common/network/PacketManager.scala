package com.rikmuld.corerm.common.network
...
object PacketSender {
  def to(data: BasicPacketData, player: EntityPlayer) = CoreObjs.network.sendTo(new PacketGlobal(data), player.asInstanceOf[EntityPlayerMP])
  def toServer(data: BasicPacketData) = CoreObjs.network.sendToServer(new PacketGlobal(data))
  def toClient(data: BasicPacketData) = CoreObjs.network.sendToAll(new PacketGlobal(data))
}
